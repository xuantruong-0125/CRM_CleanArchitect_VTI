...
import org.example.crm_project.modules.leads_managerment.application.mapper.LeadReferenceMapper;
import org.example.crm_project.modules.leads_managerment.domain.exception.InvalidLeadException;
import org.example.crm_project.modules.leads_managerment.domain.repository.LeadReferenceCatalogRepository;
    ...
    public LeadReferenceCatalogResponse getReferenceCatalog() {
        return LeadReferenceCatalogResponse.builder()
                ...
                .sources(leadReferenceCatalogRepository.findLeadSources().stream()
                        .map(LeadReferenceMapper::toResponse)
                        .toList())
                .provinces(leadReferenceCatalogRepository.findProvinces().stream()
                        .map(LeadReferenceMapper::toResponse)
                        .toList())
