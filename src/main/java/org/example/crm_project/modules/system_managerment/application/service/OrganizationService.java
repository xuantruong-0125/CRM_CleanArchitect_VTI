...
import org.example.crm_project.modules.system_managerment.application.mapper.OrganizationMapper;
...
import org.example.crm_project.modules.system_managerment.domain.exception.OrganizationNotFoundException;
...
public class OrganizationService {
    ...
    public List<OrganizationResponse> getTree() {
        ...
        Map<Long, OrganizationResponse> map = new HashMap<>();

        for (Organization org : all) {
            map.put(org.getId(), OrganizationMapper.toResponse(org));
        }

        List<OrganizationResponse> roots = new ArrayList<>();

        for (Organization org : all) {
            OrganizationResponse node = map.get(org.getId());

            if (org.getParentId() == null) {
                roots.add(node);
            } else {
                OrganizationResponse parent = map.get(org.getParentId());

                if (parent != null) {
                    parent.getChildren().add(node);
                } else {
                    roots.add(node); // fallback nếu parent lỗi
                }
            }
        }
