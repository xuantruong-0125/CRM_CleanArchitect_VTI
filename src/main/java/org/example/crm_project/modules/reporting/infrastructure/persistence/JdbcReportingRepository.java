...
public class JdbcReportingRepository implements ReportingRepository {
    ...
    private List<AssignmentInfo> getAssignments(Long kpiConfigId) {
        String sql = "SELECT ta.user_id, ta.commission_percent, u.full_name, u.username, o.name as org_name " +
    ...
    public DashboardData getDashboardData(Long kpiConfigId, Long userId) {
        ...
        if (userId != null) {
            userIds.add(userId);
            AssignmentInfo matchingAsn = allAsn.stream().filter(a -> a.userId.equals(userId)).findFirst().orElse(null);
            if (matchingAsn != null) {
        ...
        data.setKhtnMoi(new MetricValue(totalActualKhtnMoi, targetMap.getOrDefault(ReportingConstants.MT_KHTN_MOI, BigDecimal.ZERO)));
        data.setKhtnLienHe(new MetricValue(totalActualLienHe, targetMap.getOrDefault(ReportingConstants.MT_KHTN_LIEN_HE, BigDecimal.ZERO)));
        data.setKhtnChuyenDoi(new MetricValue(totalActualChuyenDoi, targetMap.getOrDefault(ReportingConstants.MT_KHTN_CHUYEN_DOI, BigDecimal.ZERO)));
        data.setKhMoiMua(new MetricValue(totalActualMoiMua, targetMap.getOrDefault(ReportingConstants.MT_KH_MOI_MUA, BigDecimal.ZERO)));
        data.setKhCuQuayLai(new MetricValue(totalActualCuQuayLai, targetMap.getOrDefault(ReportingConstants.MT_KH_CU_QUAY_LAI, BigDecimal.ZERO)));
        data.setCuocGoi(new MetricValue(totalActualCuocGoi, targetMap.getOrDefault(ReportingConstants.MT_CUOC_GOI, BigDecimal.ZERO)));
        data.setCuocGap(new MetricValue(totalActualCuocGap, targetMap.getOrDefault(ReportingConstants.MT_CUOC_GAP, BigDecimal.ZERO)));
        data.setCuocGoiTongDai(new MetricValue(totalActualTongDai, targetMap.getOrDefault(ReportingConstants.MT_CUOC_GOI_TONG_DAI, BigDecimal.ZERO)));
        data.setEmailBaoGia(new MetricValue(totalActualEmailBaoGia, targetMap.getOrDefault(ReportingConstants.MT_EMAIL_BAO_GIA, BigDecimal.ZERO)));
        data.setEmailGuiKh(new MetricValue(totalActualEmailGuiKh, targetMap.getOrDefault(ReportingConstants.MT_EMAIL_GUI_KH, BigDecimal.ZERO)));

        data.setTotalDoanhSo(totalActualDoanhSo);
        data.setTargetDoanhSo(targetMap.getOrDefault(ReportingConstants.MT_DOANH_SO, BigDecimal.ZERO));
        data.setTotalHoaHong(totalActualDoanhSo.multiply(commissionPercent).divide(BigDecimal.valueOf(100), 0, RoundingMode.HALF_UP));

        BigDecimal targetLienHe = targetMap.getOrDefault(ReportingConstants.MT_KHTN_LIEN_HE, BigDecimal.ZERO);
        BigDecimal totalKhtnMoiVaLienHe = targetMap.getOrDefault(ReportingConstants.MT_KHTN_MOI, BigDecimal.ZERO).add(targetLienHe);
        ...
        BigDecimal targetChuyenDoi = targetMap.getOrDefault(ReportingConstants.MT_KHTN_CHUYEN_DOI, BigDecimal.ZERO);
        ...
        BigDecimal targetDs = targetMap.getOrDefault(ReportingConstants.MT_DOANH_SO, BigDecimal.ZERO);
    ...
    private BigDecimal computeActual(String metricType, Long userId, LocalDate startDate, LocalDate endDate) {
        try {
            ...
            return switch (metricType) {
                case ReportingConstants.MT_KHTN_MOI -> countLeadsByStatus(userId, "NEW", start, end);
                case ReportingConstants.MT_KHTN_LIEN_HE -> countLeadsByStatus(userId, "CONTACTING", start, end);
                case ReportingConstants.MT_KHTN_CHUYEN_DOI -> countLeadsByStatus(userId, "CONVERTED", start, end);
    ...
    private BigDecimal countLeadsByStatus(Long userId, String status, String start, String end) {
        String sql = "SELECT COUNT(*) FROM leads WHERE assigned_to = ? AND status = ? AND created_at >= ? AND created_at <= ? AND deleted_at IS NULL";
        Long count = jdbcTemplate.queryForObject(sql, Long.class, userId, status, start, end + " 23:59:59");
