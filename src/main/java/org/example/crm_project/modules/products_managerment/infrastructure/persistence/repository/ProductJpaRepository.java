...
public interface ProductJpaRepository extends JpaRepository<ProductEntity, Long> {
    ...
    @Query("SELECT p FROM ProductsProductEntity p WHERE p.isActive = true AND p.deletedAt IS NULL AND p.categoryEntity.isActive = true AND p.categoryEntity.deletedAt IS NULL")
    List<ProductEntity> findAllActive();

    @Query("SELECT p FROM ProductsProductEntity p WHERE p.isActive = true AND p.deletedAt IS NULL AND p.categoryEntity.isActive = true AND p.categoryEntity.deletedAt IS NULL")
    Page<ProductEntity> findAllActive(Pageable pageable);

    @Query("SELECT p FROM ProductsProductEntity p " +
    ...
    @Query("SELECT COUNT(DISTINCT p.id) FROM ProductsProductEntity p " +
           "LEFT JOIN p.prices pr ON (pr.isActive = true AND pr.deletedAt IS NULL) " +
           "WHERE (:keyword IS NULL OR :keyword = '' OR p.name LIKE CONCAT('%', :keyword, '%') OR p.skuCode LIKE CONCAT('%', :keyword, '%') OR p.description LIKE CONCAT('%', :keyword, '%')) " +
           "AND (:categoryId IS NULL OR p.categoryEntity.id = :categoryId) " +
           "AND (:minPrice IS NULL OR pr.finalPrice >= :minPrice) " +
    ...
    @Query("SELECT p.categoryEntity.id, COUNT(p) FROM ProductsProductEntity p WHERE p.isActive = true AND p.deletedAt IS NULL AND p.categoryEntity.isActive = true AND p.categoryEntity.deletedAt IS NULL GROUP BY p.categoryEntity.id")
    List<Object[]> countActiveProductsGroupedByCategory();

    @Query("SELECT COUNT(p) > 0 FROM ProductsProductEntity p WHERE p.skuCode = :skuCode AND p.isActive = true AND p.deletedAt IS NULL AND p.categoryEntity.isActive = true AND p.categoryEntity.deletedAt IS NULL")
    boolean existsBySkuCodeAndIsActiveTrue(@Param("skuCode") String skuCode);

    @Query("SELECT COUNT(p) > 0 FROM ProductsProductEntity p WHERE p.skuCode = :skuCode AND p.id != :id AND p.isActive = true AND p.deletedAt IS NULL AND p.categoryEntity.isActive = true AND p.categoryEntity.deletedAt IS NULL")
